...
sapColor = {
    ...
    "teal3": hex2rgb("33e99d"),
    
    "background": hex2rgb("1a2345"),
    "untitledBackground": hex2rgb("171826")
};
...
duration(textObject) := textObject.length / writingSpeed;

forall(keys(sapColor),
    parse(# + " = " + sapColor_# + ";");
);

forall(keys(sapColorHex),
    parse(# + "Hex = " + sapColorHex_# + ";");
);
